...
function parseGlobalSorts(globalFilter) {
	const currentSorts = Array.isArray(globalFilter?.sort)
		? globalFilter.sort
		: [];
...
function ValueInput({ field, value, onChange }) {
	...
	switch (ctx.type) {
		case "select": {
			const safeValue = ctx.options.some((o) => o.value === value)
				? value
				: (ctx.options[0]?.value ?? "");
...
function FilterRow({ filter, setFilter, onDelete }) {
	...
	const currentOp = validOperators.includes(filter.op)
		? filter.op
		: validOperators[0];
	...
	return (
		<Box sx={{ display: "flex", gap: 0, mt: 1, pl: 2 }}>
			<Select
				...
				sx={{
					borderRadius:
						"var(--joy-radius-sm) 0 0 var(--joy-radius-sm)",
					boxShadow: "none",
				}}
				variant="soft"
			>
			...
			<IconButton
				...
				sx={{
					borderRadius:
						"0 var(--joy-radius-sm) var(--joy-radius-sm) 0",
				}}
			>
				<ClearIcon />
			</IconButton>
		...
		<>
			<Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
				<Select
					...
					onChange={(e, newValue) =>
						setGroup({ ...group, operator: newValue })
					}
					variant="soft"
					size="sm"
				>
...
export default function MoreFilterModal({ open, setOpen }) {
	...
	const [sorts, setSorts] = useState(() =>
		parseGlobalSorts(initialGlobalFilter),
	);
	...
	const saveFilter = (newValue) => {
		...
		if (existingFilter) {
			notify(`This filter already exists as "${existingFilter}"`, {
				color: "danger",
			});
			setSavedFiltersSearchValue("");
			return;
				...
				<ModalDialog sx={{ maxWidth: 600, width: "100%" }}>
					...
					<DialogContent sx={{ gap: 1.5 }}>
						<Box>
							...
							<Box>
								<Autocomplete
									placeholder="Search or add a preset..."
								...
								{sorts.map((s, i) => (
									...
									>
										...
										<Select
											...
											variant="soft"
										>
											{Object.entries(fieldContext).map(
												([key, ctx]) =>
													ctx.canSort ? (
														<Option
															key={key}
						...
						<FormControl>
							...
							/>
							<FormHelperText>
								Only show assets you have saved.
							</FormHelperText>
						</FormControl>
						<Tooltip
							...
							arrow
						>
							<Button disabled={serializedRootGroup.tooLong}>
								Apply Filters
							</Button>
						</Tooltip>
					</DialogContent>
