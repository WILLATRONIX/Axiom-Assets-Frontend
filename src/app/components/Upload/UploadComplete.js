...
const UploadComplete = ({
			...
			switch (uploadType) {
				case 0:
					if (uploadItems[0].type === 1) {
						const success = await uploadWithoutDetails(
							uploadItems[0],
						);
						if (!success) break;
					} else {
	...
	const mainUploadLoop = async () => {
		...
		const findFreeSlot = () =>
			activeUploads.findIndex((slot) => slot === null);
		...
		const startUploadAtSlot = (slotIndex) => {
			...
			const promise = uploadWithoutDetails(
				nextItem,
				remaining.length === 1,
			).then(() => {
				...
				setActiveUpload(
					activeUploads
						.map((item) => (item ? item.fileName : null))
						.filter(Boolean),
				);
			});
		...
		while (
			remaining.length > 0 ||
			activeUploads.some((slot) => slot !== null)
		) {
			let slot;
	...
	const uploadWithDetails = async (item) => {
		...
		const response = await post(
			`${process.env.NEXT_PUBLIC_API_URL}/upload/asset`,
			assetForm,
		);
		...
		const { type: thumbType, value: thumbValue } =
			getThumbnailData(thumbnails);
		...
		if (response.ok && thumbType) {
			...
			thumbForm.append(
				"editData",
				JSON.stringify({ thumbnail: thumbValue, ...uploadDetails }),
			);

			await post(
				`${process.env.NEXT_PUBLIC_API_URL}/asset/edit/${response.data.uuid}`,
				thumbForm,
			);
		}
	...
	const uploadAll = async () => {
		...
		const body = uploadItems.map(
			({ thumbnail, ...uploadData }) => uploadData,
		);
		...
		if (uploadDetails?.thumbnails) {
			const renamedUploadDetails = {
				...uploadDetails,
				thumbnail: uploadDetails.thumbnails[0],
				thumbnails: [],
			};
			const edits = ["thumbnail"];
			...
			>
				<Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
					...
					>
						...
						>
							Upload{" "}
							{uploadDetails?.header ||
								`${uploadItems.length} Assets`}
						</Typography>
						...
						>
							{Array.isArray(activeUpload) &&
							activeUpload.length > 0 ? (
									...
									{uploadType !== 1 &&
										status === "complete" && (
											<Button
												...
												onClick={() => {
													router.push(
														`/u/${user.username}/${uploadedItemUUID}`,
													);
												}}
											>
												View Asset
