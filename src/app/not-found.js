...
const pathDescriptionMap = [
	...
	},
	{
		...
		description: ({ username }) =>
			`They may be using a private account or have changed their username.`,
		redirect: "/browse",
		forceRedirect: false,
	},
	{
		...
		description: ({ username }) =>
			`The asset may no longer be public or has been given a new URL.`,
		redirect: ({ username }) => `/u/${username}`,
		forceRedirect: false,
	},
];
		...
		if (params) {
			return {
				...
				description:
					...
					descriptionDefaults.description,
				redirect:
					typeof entry.redirect === "function"
						? entry.redirect(params)
						: entry.redirect,
